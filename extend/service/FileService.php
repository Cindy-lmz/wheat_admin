...
class FileService
{
    ...
    public static function getMines()
    {
        ...
        if (empty($mines)) {
            $content = file_get_contents('https://svn.apache.org/repos/asf/httpd/httpd/trunk/docs/conf/mime.types');
     ...
     * @throws \think\exception\PDOException
     */
    public static function getUploadOssDirectory()
    {
        return sysconf('storage_oss_directory');
    }
     ...
     * @return string
     */
    public static function getUriLocal()
    {
        $appRoot = request()->root(true);  // 去掉参数 true 将获得相对地址
        $uriRoot = preg_match('/\.php$/', $appRoot) ? dirname($appRoot) : $appRoot;
        $uriRoot = in_array($uriRoot, ['/', '\\']) ? '' : $uriRoot;
