 ...
 * @method \WeChat\Basic WeChatBasic() static 微信基础配置管理
 * @method \WeChat\Basic WeChatInvoice() static 微信基础配置管理
 *
 *
 * ----- WeMini -----
...
class WechatService
{
     ...
     */
    public static function instance($name, $type = null,$aid = null)
    {
        ...
        if ($aid !=null) {
            ...
            $ress = Db::name('system_wechatconfig')->where('aid',$aid)->select();

            if (!empty($ress)) {
            
                session('usercofig',$ress[0]);
            }
         }
        $res = session('usercofig');
         if ($res != null) {
             $wechat_type = 'Api';
         }else{
            $wechat_type =sysconf('wechat_type',null,$aid);
         }

         
        switch (strtolower($wechat_type)) {

            case 'api':
            if (!empty($res)) {
                
               $config = [
                    'token'          => $res['wechat_token'],
                    'appid'          => $res['wechat_appid'],
                    'appsecret'      => $res['wechat_appsecret'],
                    'encodingaeskey' => $res['wechat_encodingaeskey'],
                    'mch_id'         => $res['wechat_mch_id'],
                    'mch_serialno'   => $res['wechat_mch_serialno'],
                    'mch_key'        => $res['wechat_partnerkey'],
                    'ssl_cer'        => self::_parseCertPath($res['wechat_cert_cert']),
                    'ssl_key'        => self::_parseCertPath($res['wechat_cert_key']),
                    'ssl_p12'        => self::_parseCertPath($res['wechat_mch_ssl_p12']),
                    'certificate_serialno'        => $res['wechat_certificate_serial_no'],
                    'certificate_effectivetime'        => $res['wechat_certificate_effective_time'],
                    'certificate_expiretime'        => $res['wechat_certificate_expire_time'],
                ...
                $config = [
                    ...
                    'mch_serialno'   => sysconf('wechat_mch_serialno'),
                    'mch_key'        => sysconf('wechat_partnerkey'),
                    'ssl_cer'        => self::_parseCertPath(sysconf('wechat_cert_cert')),
                    'ssl_key'        => self::_parseCertPath(sysconf('wechat_cert_key')),
                    'ssl_p12'        => self::_parseCertPath(sysconf('wechat_mch_ssl_p12')),
                    'certificate_serialno'        => sysconf('wechat_certificate_serial_no'),
                    'certificate_effectivetime'        => sysconf('wechat_certificate_effective_time'),
                    'certificate_expiretime'        => sysconf('wechat_certificate_expire_time'),
                    'cachepath'      => env('cache_path') . 'wechat' . DIRECTORY_SEPARATOR,
                ];
            ...
            case 'thr':
                list($appid, $appkey) = [$res['wechat_appid'], $res['wechat_appsecret']];
    ...
    public static function getAppid()
    {
        if (empty(session('usercofig'))) {
            ...
            switch (strtolower(session('usercofig.wechat_type'))) {
            case 'api':
                return session('usercofig.wechat_appid');
            case 'thr':
                return session('usercofig.wechat_thr_appid');
            default:
                throw new Exception('请在后台配置微信对接授权模式！');
            }
        }
    ...
    public static function __callStatic($name, $arguments)
    {
        ...
        //添加第三个参数 Aid，管理多公众号时每个公众号的唯一ID
        return self::instance($name,'',$arguments?$arguments[0]:'');
    }
