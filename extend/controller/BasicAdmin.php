...
class BasicAdmin extends Controller
{
    ...
    protected function _list($dbQuery = null, $isPage = true, $isDisplay = true, $total = false, $result = [])
    {
        ...
        if ($isPage) {
            ...
            if (($totalNum = $page->total()) > 0) {
                ...
                $html = "<div class='pagination-container nowrap'><span>共 {$totalNum} 条记录，每页显示 <select data-auto-none>" . join('', $rowHTML) . "</select> 条，共 {$maxNum} 页当前显示第 {$curPage} 页。</span>{$page->render()}</div>";
                list($result['total'], $result['list'], $result['page']) = [$totalNum, $page->all(), preg_replace('|href="(.*?)"|', 'data-open="$1" onclick="return false" href="$1"', $html)];
            } else {
     ...
     * @throws \think\Exception
     */
    protected function _listdata($dbQuery = null, $result = [])
    {
        $db = is_null($dbQuery) ? Db::name($this->table) : (is_string($dbQuery) ? Db::name($dbQuery) : $dbQuery);
        ...
        if ($this->request->isAjax()) {
            ...
            $rows = intval($this->request->get('limit'));

            $list = $db->paginate($rows, $total, ['query' => $query]);
        ...
        if (false !== $this->_callback('_data_filter', $result['data'], [])) {
